...
(function($) {
  $.fn.millerColumn = function(options) {
    ...
    if (typeof args[0] != "string") settings = $.extend(settings, options);
    ...
    if ("addCol" == args[0]) {
      if (!isInitialized.call(this)) {
        if (isDebugEnabled) {
          console.log(
            "element is is not initialized as a miller column. You need to call jQuery('#id).millerColumn() first."
          );
        }
        return;
      }
      ...
      if (args[1].jquery) {
      ...
      } else if (args[1].categoryId) {
        //args[1] to detect if it is a valid category obj
        ...
        try {
          ...
          var lastVisibleNonColLoadingContainer = lastVisibleCol.is(
            ".col-loading"
          )
            ? lastVisibleCol.prev()
            : lastVisibleCol;
          ...
          if (
            lastVisibleNonColLoadingContainer.length !== 0 &&
            category.categoryId ===
              lastVisibleNonColLoadingContainer.data("category-id")
          ) {
          ...
          var selectedListItem = lastVisibleNonColLoadingContainer
            .find(getColListItemSelector())
            .filter(".selected");
          ...
          for (var j = 0; category.items && j < category.items.length; ++j) {
            ...
            if (
              ...
              selectedListItem.data("item-id") != item.parentId
            ) {
              console.warn(
                "Response expired; current selected item is not same as the parent of provided list item."
              );
              hideLoadingCol.call(this);
              return;
            }
        ...
        if (readOnly && category.items.length == 0) return;
        ...
        var millerColTitle = $("<div/>"),
          millerColTitleText = $("<div>")
            .addClass("miller-col-title-text")
            .append($("<span/>").text(category.categoryName));

        if (!readOnly) {
          var millerColAction = $("<span/>")
      ...
      var listParentAddedItem = $(this)
        .find(getColListItemSelector())
        .filter("[data-item-id = '" + addedItemData.parentId + "']");

      if (null != listParentAddedItem) {
        var parentAlreadyWithChildren = listParentAddedItem.data(
          "has-children"
        );
      ...
      var colContainer = getMillerColContainers
        .call(this)
        .filter("[data-category-id='" + addedItemData.categoryId + "']");

      if (null == colContainer) {
        if (isDebugEnabled) {
          console.log(
            "Cant find category with id: " + addedItemData.categoryId
          );
        }
        return;
      }
      ...
      var listUpdatedItem = $(this)
        .find(getColListItemSelector())
        .filter("[data-item-id = '" + updatedtemData.itemId + "']");
      ...
      var listDeletedItem = $(this)
        .find(getColListItemSelector())
        .filter("[data-item-id = '" + deletedItemData.itemId + "']");

      var listParentItem = $(this)
        .find(getColListItemSelector())
        .filter("[data-item-id = '" + deletedItemData.parentId + "']");

      if (null != listParentItem) {
        var childrenLeft = $(this)
          .find(getColListItemSelector())
          .filter("[data-parent-id = '" + deletedItemData.parentId + "']");
      ...
      $(listDeletedItem)
        .closest(getColContainerSelector())
        .next(getColContainerSelectorExcludeColLoading())
        .remove();

      if (null != listDeletedItem) listDeletedItem.remove();
      ...
      if (isInitialized.call(this)) {
        ...
        if (isDebugEnabled) console.log("miller-column destroyed !");
      } else {
        if (isDebugEnabled)
          console.log(
            "miller-column is not initialized so nothing to destroy ..."
          );
      }
    ...
    function buildColListItem(item, readOnly) {
      ...
      if (item.numChildren != null && item.numChildren != 0) {
        millerColListItem.append(
          $("<span/>")
            .addClass("num-children-badge")
            .text(item.numChildren)
        );
      }
    ...
    function getTotalInnerColsWidth() {
      ...
      getMillerColContainers.call(this).each(function() {
        if (isViewHidden.call(this)) return;
      ...
      totalWidth =
        totalWidth +
        (!isViewHidden.call(getLoadingCol.call(this))
          ? getLoadingCol.call(this).outerWidth()
          : 0);
    ...
    function initialize() {
      if (getMillerColsBody.call(this).length == 0) {
        //if user has not build the miller ui structure using html manually
        ...
        var millerColsBody = $("<div/>").addClass("miller-cols-body");
        ...
        var millerColLoading = $("<div/>")
          ...
          .append(
            $("<div/>")
              ...
              .append(
                $("<div/>").addClass(
                  "miller-col-list-item loading-icon-container spinner"
                )
              )
          );
    ...
    function getFirstVisibleCol() {
      var firstVisibleCol = getMillerColsBody
        .call(this)
        .find(".miller-col-container:not(.hidden):first");

      return firstVisibleCol.length != 0
        ? firstVisibleCol
        : isViewHidden.call(getLoadingCol.call(this))
          ? null
          : getLoadingCol.call(this);
    }
    ...
    function hasRightHiddenCol() {
      return isViewHidden.call(
        getLastVisibleCol
          .call(this)
          .next()
          .not(getColLoadingSelector())
      );
    }

    function getLastVisibleCol() {
      return !isViewHidden.call(getLoadingCol.call(this))
        ? getLoadingCol.call(this)
        : getMillerColsBody
            .call(this)
            .find(".miller-col-container:not(.hidden):last");
    }
    ...
    function hideView() {
      if (!isViewHidden.call(this)) $(this).addClass(MILLER_COL_HIDDEN_CLASS);
    }

    function showView() {
      if (isViewHidden.call(this)) $(this).removeClass(MILLER_COL_HIDDEN_CLASS);
    }
    ...
    function isOverFlowing() {
      ...
      return (
        getMillerColsBody.call(this).outerWidth() + 1 <
        getTotalInnerColsWidth.call(getMillerColsBody.call(this))
      );
    }
    ...
    function overFlowsWith(child) {
      ...
      return (
        isOverFlowing.call(this) ||
        millerColsBody.outerWidth() + 1 <
          getTotalInnerColsWidth.call(millerColsBody) + child.outerWidth()
      );
    }
    ...
    return this.each(function() {
      ...
      if (args[0] && args[0]["initData"]) {
        //arg[0] root col

        //hide all cols except col load b/se arg[0] => root col
        $(millerColumn)
          .find(getColContainerSelectorExcludeColLoading())
          .remove();
      ...
      getMillerColsBody
        ...
        .on("DOMNodeInserted", function(event) {
          ...
          if (!$(event.target).is(getColContainerSelector())) return;
      ...
      getMillerColsBody
        .call(millerColumn)
        .on("click", getColListItemSelector(), function() {
      ...
      getMillerColsBody
        .call(millerColumn)
        .on("click", ".list-item-actions .edit", function(event) {
      ...
      getMillerColsBody
        .call(millerColumn)
        .on("click", ".list-item-actions .delete", function(event) {
          var currentItemContainer = $(this).closest(getColListItemSelector());
      ...
      getMillerColsBody
        .call(millerColumn)
        .on("click", ".miller-col-actions .action-add", function() {
          var currentColContainer = $(this).closest(getColContainerSelector());
          ...
          if (parentColContainer)
            data.parentId = $(parentColContainer)
              .find(getColListItemSelector())
              .filter(SELECTOR_IS_SELECTED)
              .data("item-id");
      ...
      $(window).on("resize." + $(millerColumn).attr("id"), function(event) {
        waitForFinalEvent(
          function() {
            if (!isInitialized.call(millerColumn)) return;
